...
internal class PopupHeader : CView
{
    ...
    {
        if (HeaderView is PopupGrid sfGrid && base.Children.Count > 0)
        {
    ...
    {
        PopupGrid sfGrid = HeaderView as PopupGrid;
        ...
        if (sfGrid != null)
        {
            if (canShowPopupInFullScreen)
            {
                if (PopupView.Popup.ShowCloseButton)
                {
                    ...
                    sfGrid.Padding = ((PopupView.Popup.HeaderTemplate == null) ? new Thickness(headerPadding, headerPadding, headerViewPadding, headerViewPadding) : new Thickness(0.0));
                ...
                {
                    sfGrid.Padding = ((PopupView.Popup.HeaderTemplate == null) ? new Thickness(headerViewPadding) : new Thickness(0.0));
                }
            ...
            if (PopupView?.Popup.HeaderTemplate == null)
            {
                ...
                sfGrid.Children.Add(TitleLabel);
                Grid.SetColumn((BindableObject)TitleLabel, (PopupView.Popup.ShowCloseButton && canShowPopupInFullScreen) ? 1 : 0);
            }
            else
            {
                View view = (View)PopupView.Popup.GetTemplate(PopupView.Popup.HeaderTemplate).CreateContent();
            ...
            if (PopupView.Popup.ShowCloseButton)
            {
                sfGrid.Children.Add(PopupCloseButton);
    ...
    private void UpdateHeaderChildView()
    {
        ...
        {
            PopupGrid sfGrid = HeaderView as PopupGrid;
            View view = sfGrid.Children[0] as View;
            bool canShowPopupInFullScreen = PopupView.Popup.CanShowPopupInFullScreen;
            double num = (PopupView.Popup.ShowCloseButton ? sfGrid.ColumnDefinitions[(!canShowPopupInFullScreen) ? 1 : 0].Width.Value : 0.0);
